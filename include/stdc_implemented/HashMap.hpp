...
public:
	...
	void put(KeyType key, ValType val) {
		...
		unsigned int index = toHash(move(key)) % bucket->bucket_size;
		...
		// 没找到键值对，则将键值对添加至末尾
		chain.add(typename Bucket::Chain::DataPair(move(key), move(val)));
...
using HashMap = IHashMap<KeyType, ValType, stdc::HashMap<KeyType, ValType>>;
